...
internal class AddStageDialog
{
    ...
    public AddStageDialog(MainWindowContext window, bool resetOnDone = true)
    {
        ...
        comboStrings = comboStrings.Concat(["Special 1", "Special 2", "Special 3", "Special 4", "Special 5", "Special 6", "Special 7", "Special 8"]).ToArray();
    ...
    public void Render()
    {
        ...
        {
            _foundStages = _window.ContextHandler.FSHandler.OriginalFS.EnumerateStages().Where(t => t.Name.Contains(_name)).ToList();
        ...
        else
        {
            ...
            if (ImGui.BeginListBox("##stageList") && _window.ContextHandler.FSHandler.ReadGameSystemDataTable() is not null)
            {

                foreach (SystemDataTable.StageDefine _stage in _window.ContextHandler.FSHandler.ReadGameSystemDataTable().WorldList[currentItem - 1].StageList)
                {
                    if (_stage.StageType == SystemDataTable.StageTypes.Empty || _stage.StageType == SystemDataTable.StageTypes.Dokan)
                        continue;
